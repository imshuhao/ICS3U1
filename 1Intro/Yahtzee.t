...
procedure putdata (data1, data2, data3, data4, data5, data6 : int)
    ...
    put "---------------"
    locate (19, 4)
    put "Total Points:", data1 + data2 + data3 + data4 + data5 + data6 : 3
    if rolledcounter = 12 then
	locate (13, 25)
	put "Game Finished! Your final points is", data1 + data2 + data3 + data4 + data5 + data6 : 3, '.'
    end if
end putdata
...
loop
    ...
    rolledcounter += 1
    if rolledcounter mod 2 = 0 and rolledcounter not= 12 then
	locate (2, 5)
	put "Roll ", rolledcounter, ": Click ROLL to reset.":40 ..
    elsif rolledcounter = 12 then
	locate (2, 5)
	put "Roll ", rolledcounter, ": Game finished.":40 ..
    else
	locate (2, 5)
	put "Roll ", rolledcounter, ": Select a number by clicking on a die.":40 ..
    end if
    if rolledcounter mod 2 = 0 then
	selectrestdice (dicenumber)
	putdata (data1, data2, data3, data4, data5, data6)
